...
void GLViewNewModule::onCreate()
{
   ...
   if ((_access("../irrKlang-64bit-1.6.0/media/Tranquility.mp3", 0)) == -1) {
       ...
       std::cout << "Should put the 'Debug' folder under 'NewModule' directly." << std::endl;
       return;
   }
   ...
   std::cout << this->snd->getSound3D().size() << std::endl;
...
void GLViewNewModule::updateWorld()
{
   GLView::updateWorld(); //Just call the parent's update world first.
                          ...
                          //this call.
   if (!this->snd->getSound3D().empty()) {
       this->snd->getSoundEngine()->setListenerPosition(
           this->snd->convertFromVector(this->cam->getPosition()),
           this->snd->convertFromVector(this->cam->getLookDirection()),
           irrklang::vec3df(0, 0, 0),
           this->snd->convertFromVector(this->cam->getNormalDirection())
...
void GLViewNewModule::onKeyDown( const SDL_KeyboardEvent& key )
{
   ...
   if (key.keysym.sym == SDLK_p) {
       if (!this->snd->getSound2D().empty() && !this->snd->getSound2D().at(0)->isFinished()) {
           this->snd->stopSound2D();
       }
       else {
           this->snd->play2D("../irrKlang-64bit-1.6.0/media/Tranquility.mp3", true, false, true);
       }
...
void GLViewNewModule::createNewModuleWayPoints()
{
   ...
   wayPt->setPosition( Vector( 50, 0, 50 ) );
